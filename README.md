...
| Editing bindings, the leader chord, and the TUI's own keys, from the TUI or CLI | yes — kinds, actions and arguments are chosen from what the daemon accepts; `ctrl+r` records a key as the trigger; written back to `config.toml` with comments preserved |
